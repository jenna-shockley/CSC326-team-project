...
public class AddUserStepDefs {
    ...
    private final String    labtechUname = "testTech";
    ...
    /**
     * Check that testTech isn't in the system.
     */
    @Given ( "The labtech does not already exist in the database" )
    public void noLabtech () {
        final List<User> users = User.getUsers();
        for ( final User user : users ) {
            if ( user.getUsername().equals( labtechUname ) ) {
                try {
                    user.delete();
                }
                catch ( final Exception e ) {
    ...
    /**
     * Fill in add user values for labtech
     */
    @When ( "I fill in the values with the role as labtech in the Add User form" )
    public void fillLabtechFields () {
        final WebElement username = driver.findElement( By.id( "username" ) );
        username.clear();
        username.sendKeys( labtechUname );
     ...
     * Labtech login
     */
    @Then ( "The new labtech can login" )
