...
public class OfficeVisit extends DomainObject<OfficeVisit> {
    ...
    public OfficeVisit ( final OfficeVisitForm ovf ) throws ParseException, NumberFormatException {
        ...
        final List<LabProcForm> lp = ovf.getLabProcs();
        if ( lp != null ) {
            setLabProcs(
                    lp.stream().map( ( final LabProcForm lpf ) -> new LabProc( lpf ) ).collect( Collectors.toList() ) );
        }
    }
    ...
    /**
     * Sets the list of lab procedures for this visit.
     *
     * @param labProcs
     *            the list of lab procedures
     */
    public void setLabProcs ( final List<LabProc> labProcs ) {
        this.labProcs = labProcs;
    }

    /**
     * Returns the list of lab procedures associated with this office visit.
     *
     * @return lab procedures the list lab procedures
     */
    ...
    /** Lab procedures for the visit */
    @OneToMany ( )
    @LazyCollection ( LazyCollectionOption.FALSE )
    @JoinColumn ( name = "labProcs_id" )
    private List<LabProc>            labProcs      = Collections.emptyList();
    ...
    public void save () {
        ...
        // Get saved visit
        OfficeVisit oldVisit = OfficeVisit.getById( id );
        ...
        this.getPrescriptions().forEach( p -> {
            ...
            final boolean isNew = currentIds.contains( p.getId() ) && !savedIds.contains( p.getId() );
            if ( isSaved && isNew ) {
        ...
        // Get saved visit
        oldVisit = OfficeVisit.getById( id );

        // Get lab procedures ids included in this office visit
        final Set<Long> currentLPIds = this.getLabProcs().stream().map( LabProc::getId ).collect( Collectors.toSet() );

        // Get lab procedures ids saved previously
        final Set<Long> savedLPIds = oldVisit == null ? Collections.emptySet()
                : oldVisit.getLabProcs().stream().map( LabProc::getId ).collect( Collectors.toSet() );

        // Save each of the lab procedures
        this.getLabProcs().forEach( lp -> {
            final boolean isNew = currentLPIds.contains( lp.getId() ) && !savedLPIds.contains( lp.getId() );
            if ( isNew ) {
                LoggerUtil.log( TransactionType.ADD_LAB_PROC, LoggerUtil.currentUser(), getPatient().getUsername(),
                        "Creating lab procedure with id " + lp.getId() );
                LoggerUtil.log( TransactionType.ADD_LAB_PROC, LoggerUtil.currentUser(), lp.getLabtech().getUsername(),
                        "Creating lab procedure with id " + lp.getId() );
            }
            lp.save();
        } );
        ...
        if ( !savedLPIds.isEmpty() ) {
            savedLPIds.forEach( id -> {
                final boolean isMissing = !currentLPIds.contains( id );
                if ( isMissing ) {
                    LoggerUtil.log( TransactionType.DELETE_LAB_PROC, LoggerUtil.currentUser(),
                            getPatient().getUsername(), "Deleting lab procedure with id " + id );
                    final LabProc lp = LabProc.getById( id );
                    LoggerUtil.log( TransactionType.DELETE_LAB_PROC, LoggerUtil.currentUser(),
                            lp.getLabtech().getUsername(), "Deleting lab procedure with id " + id );
                    lp.delete();
                }
            } );
        }
