...
public class APIUserController extends APIController {
    ...
    /** constant for responder role */
    private static final String ROLE_RESPONDER = "ROLE_RESPONDER";
    ...
    /**
     * Retrieves and returns a list of all Labtechs in the system
     *
     * @return list of lab techs
     */
    @GetMapping ( BASE_PATH + "/labtechs" )
    public List<String> getLabtechs () {
        final List<User> labtechs = User.getByRole( Role.ROLE_LABTECH );
        final List<String> labtechsWithNum = new ArrayList<String>();
        for ( int i = 0; i < labtechs.size(); i++ ) {
            int size = 0;
            try {
                size = LabProc.getAllForLabtech( labtechs.get( i ) ).size();
            }
            ...
            line.append( labtechs.get( i ).getUsername() );
            line.append( " currently has " );
            line.append( size );
            line.append( " lab procedures." );

            labtechsWithNum.add( line.toString() );
        }
        return labtechsWithNum;
    }
    ...
    public ResponseEntity getRole () {
        ...
        else if ( hasRole( ROLE_RESPONDER ) ) {
            return new ResponseEntity( successResponse( ROLE_RESPONDER ), HttpStatus.OK );
        }
